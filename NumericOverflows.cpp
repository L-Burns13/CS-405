...
void test_overflow()
{
    ...
    // Try the calculation that may cause overflow
    try {
        result = add_numbers<T>(start, increment, steps + 1);
        std::cout << +result << std::endl;
    }
    // Show a message if overflow happens
...
void test_underflow()
{
    ...
    // Try the calculation that may cause underflow
    try {
        result = subtract_numbers<T>(start, decrement, steps + 1);
        std::cout << +result << std::endl;
    }
    // Show a message if underflow happens
