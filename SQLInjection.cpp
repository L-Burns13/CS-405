...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
	...
	// A simple regex pattern to detect common SQL injection patterns 
	std::regex injection_pattern(R"(\bor\s+('[^']*'|\d+)\s*=\s*\1)", std::regex_constants::icase);

    if (std::regex_search(sql, injection_pattern))
  {
    std::cout << "SQL Injection Detected in query: " << sql << std::endl;
    return false;
	}
